...
typedef uint32_t MDS_BUTTON_Tick_t;
typedef uint16_t MDS_BUTTON_Mask_t;

typedef struct MDS_BUTTON_Device MDS_BUTTON_Device_t;
typedef struct MDS_BUTTON_Group {
    MDS_BUTTON_Device_t *next, *prev;
} MDS_BUTTON_Group_t;

typedef struct MDS_BUTTON_InitStruct {
    const void *periph;
    MDS_BUTTON_Mask_t (*getLevel)(const void *periph);
    MDS_BUTTON_Mask_t releasedLevel;
    uint16_t debounceOut;

    MDS_BUTTON_Tick_t clickTicks;
    MDS_BUTTON_Tick_t holdTicks;
...
struct MDS_BUTTON_Device {
    ...
    MDS_BUTTON_Tick_t tickCount;
    MDS_BUTTON_Mask_t btnLevel;
    MDS_BUTTON_Mask_t fakedLevel;
    int8_t isFaked;
...
#define MDS_BUTTON_GROUP_INIT(btnGroup)                                                            \
    {.list.next = &((btnGroup)->list), .list.prev = &((btnGroup)->list)}
