...
} __attribute__((packed)) DEV_SPI_DataBits_t;

typedef enum DEV_SPI_FirstBit {
    DEV_SPI_FIRSTBIT_MSB,
    DEV_SPI_FIRSTBIT_LSB,
} __attribute__((packed)) DEV_SPI_FirstBit_t;

typedef enum DEV_SPI_BusCS {
    DEV_SPI_BUSCS_NO = -1,
    DEV_SPI_BUSCS_LOW,
    DEV_SPI_BUSCS_HIGH,
} __attribute__((packed)) DEV_SPI_BusCS_t;
...
typedef struct DEV_SPI_Driver {
    MDS_Err_t (*control)(const DEV_SPI_Adaptr_t *spi, MDS_DevCmd_t cmd, MDS_Arg_t arg);
