...
#define MDS_TIMEOUT_NOWAIT   MDS_TIMEOUT_TICKS(MDS_CLOCK_TICK_NO_WAIT)
...
MDS_TimeStamp_t MDS_ClockGetTimestamp(int8_t *tz);
void MDS_ClockSetTimestamp(MDS_TimeStamp_t ts, int8_t tz);

static inline void MDS_ClockDelayRaw(MDS_Timeout_t timeout)
{
...
static inline void MDS_ClockDelayCnt(MDS_Tick_t cnt)
{
    for (volatile MDS_Tick_t count = cnt; count > 0; count -= 1) {
    }
}
