...
static void MDS_FsLock(void)
{
    ...
    do {
        err = MDS_MutexAcquire(&g_fsLock, MDS_CLOCK_TICK_FOREVER);
    } while (err != MDS_EOK);
}
...
MDS_Err_t MDS_FileSystemUnmount(const char *path)
{
    ...
    do {
        ...
        MDS_MutexAcquire(&(fs->lock), MDS_CLOCK_TICK_FOREVER);
...
MDS_Err_t MDS_FileOpen(MDS_FileDesc_t *fd, const void *path, MDS_Mask_t flags)
{
    MDS_ASSERT(fd != NULL);
    MDS_ASSERT(path != NULL);
    ...
    do {
        ...
        fd->node = fnode;
        fd->pos = 0;
        fd->flags = flags | MDS_FILE_OFLAG_OPEN;
...
typedef MDS_FileNode_t *(*MDS_FilePath_t)(void);
...
MDS_Err_t MDS_FileOpen(MDS_FileDesc_t *fd, const void *path, MDS_Mask_t flags)
{
    ...
    MDS_FileNode_t *fileNode = ((MDS_FilePath_t)path)();
    if (fileNode == NULL) {
        return (MDS_EEXIST);
    }

    if (MDS_ObjectGetType(&(fileNode->lock.object)) != MDS_OBJECT_TYPE_MUTEX) {
        MDS_MutexInit(&(fileNode->lock), "");
    }

    if ((fileNode->fs == NULL) || (fileNode->fs->ops == NULL) || (fd->node->fs->ops->open == NULL)) {
        return (MDS_EPERM);
    }

    MDS_Err_t err = MDS_MutexAcquire(&(fileNode->lock), MDS_CLOCK_TICK_FOREVER);
    if (err == MDS_EOK) {
        fd->node = fileNode;
        fd->pos = 0;
        fd->flags = flags | MDS_FILE_OFLAG_OPEN;

        err = fd->node->fs->ops->open(fd);
        if (err != MDS_EOK) {
            fd->node = NULL;
            fd->pos = 0;
            fd->flags = MDS_FILE_OFLAG_NONE;
            MDS_MutexRelease(&(fileNode->lock));
        }
    }
...
MDS_Err_t MDS_FileClose(MDS_FileDesc_t *fd)
{
    ...
    if ((fd->node == NULL) || ((fd->flags & MDS_FILE_OFLAG_OPEN) == MDS_FILE_OFLAG_NONE)) {
        return (MDS_EACCES);
    }

    if ((fd->node->fs == NULL) || (fd->node->fs->ops == NULL) || (fd->node->fs->ops->close == NULL)) {
        return (MDS_EPERM);
    }

    MDS_Err_t err = fd->node->fs->ops->close(fd);
    if (err == MDS_EOK) {
        fd->node = NULL;
        fd->pos = 0;
        fd->flags = MDS_FILE_OFLAG_NONE;
        MDS_MutexRelease(&(fd->node->lock));
    }
...
MDS_Err_t MDS_FileIoctl(MDS_FileDesc_t *fd, MDS_Item_t cmd, MDS_Arg_t *args)
{
    ...
    if ((fd->node == NULL) || ((fd->flags & MDS_FILE_OFLAG_OPEN) == 0U)) {
        return (MDS_EACCES);
    }
...
MDS_FileOffset_t MDS_FileRead(MDS_FileDesc_t *fd, uint8_t *buff, MDS_FileSize_t len)
{
    ...
    if ((fd->node == NULL) ||
        ((fd->flags & (MDS_FILE_OFLAG_OPEN | MDS_FILE_OFLAG_RDWR)) == (MDS_FILE_OFLAG_OPEN | MDS_FILE_OFLAG_RDONLY))) {
        return (MDS_EACCES);
    }
...
MDS_FileOffset_t MDS_FileWrite(MDS_FileDesc_t *fd, const uint8_t *buff, MDS_FileSize_t len)
{
    ...
    if ((fd->node == NULL) ||
        ((fd->flags & (MDS_FILE_OFLAG_OPEN | MDS_FILE_OFLAG_RDWR)) == (MDS_FILE_OFLAG_OPEN | MDS_FILE_OFLAG_WRONLY))) {
        return (MDS_EACCES);
    }
...
MDS_FileOffset_t MDS_FileTruncate(MDS_FileDesc_t *fd, MDS_FileOffset_t len)
{
    ...
    MDS_FileOffset_t pos = fd->node->fs->ops->truncate(fd, len);
    if (fd->pos < pos) {
        fd->pos = pos;
    }
...
MDS_Err_t MDS_FileRename(const char *oldpath, const char *newpath)
{
    ...
    do {
        ...
        MDS_MutexAcquire(&(oldfs->lock), MDS_CLOCK_TICK_FOREVER);
...
MDS_Err_t MDS_FileRemove(const char *path)
{
    ...
    MDS_FileSystem_t *fs = fileNode->fs;
    if ((fs == NULL) || (fs->ops == NULL) || (fs->ops->remove == NULL)) {
        return (MDS_EPERM);
    }
    ...
    MDS_Err_t err = MDS_MutexAcquire(&(fileNode->lock), MDS_CLOCK_TICK_FOREVER);
    if (err == MDS_EOK) {
        err = fs->ops->remove(fs, fileNode->path);
        MDS_MutexRelease(&(fileNode->lock));
    }
