...
MDS_Err_t MDS_ObjectInit(MDS_Object_t *object, MDS_ObjectType_t type, const char *name)
{
    ...
    MDS_Err_t err = MDS_SpinLockAcquire(&(g_objectList[type].spinlock));
    if (err == MDS_EOK) {
        MDS_DListInsertNodePrev(&(g_objectList[type].list), &(object->node));
    }
    MDS_SpinLockRelease(&(g_objectList[type].spinlock));
...
MDS_Err_t MDS_ObjectDeInit(MDS_Object_t *object)
{
    ...
    MDS_Lock_t lock = MDS_CoreInterruptLock();
    MDS_Err_t err = MDS_SpinLockAcquire(&(g_objectList[object->type].spinlock));
    if (err == MDS_EOK) {
        MDS_DListRemoveNode(&(object->node));
        object->type = MDS_OBJECT_TYPE_NONE;
    }
    MDS_SpinLockRelease(&(g_objectList[object->type].spinlock));
    MDS_CoreInterruptRestore(lock);
