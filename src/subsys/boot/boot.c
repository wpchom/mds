 ...
 **/
/* Include ----------------------------------------------------------------- */
#include "mds/boot/boot.h"
#include "mds/algo/crc.h"
#include "mds/algo/sha2.h"
...
#ifndef CONFIG_MDS_BOOT_UPGRADE_WITH_LZMA
#define CONFIG_MDS_BOOT_UPGRADE_WITH_LZMA 0
#endif

/* Variable ---------------------------------------------------------------- */
static __attribute__((section(CONFIG_MDS_BOOT_SWAP_SECTION))) MDS_BOOT_SwapInfo_t g_bootSwapInfo;
...
/* Function ---------------------------------------------------------------- */
size_t MDS_BOOT_DeviceRead(MDS_BOOT_Device_t *dev, uintptr_t ofs, uint8_t *buff, size_t size)
{
...
size_t MDS_BOOT_DeviceWrite(MDS_BOOT_Device_t *dev, uintptr_t ofs, const uint8_t *buff, size_t size)
{
...
size_t MDS_BOOT_DeviceErase(MDS_BOOT_Device_t *dev)
{
...
static MDS_BOOT_Result_t BOOT_CheckHash(MDS_BOOT_Device_t *src, uint32_t srcOfs, uint32_t size,
                                        uint8_t hash[CONFIG_MDS_BOOT_CHKHASH_SIZE])
{
    ...
    while (size > 0) {
        ...
        size_t read = MDS_BOOT_DeviceRead(src, srcOfs, g_bootCheckBuff, len);
        if (read < len) {
...
{
    size_t read = MDS_BOOT_DeviceRead(src, srcOfs, (uint8_t *)(upgradeInfo), sizeof(*upgradeInfo));
    if (read < sizeof(*upgradeInfo)) {
                                              ...
                                              const MDS_BOOT_UpgradeInfo_t *upgradeInfo)
{
    ...
    for (uint16_t i = 0; i < cnt; i++) {
        ...
        size_t read = MDS_BOOT_DeviceRead(src, srcOfs, (uint8_t *)(&binInfo), sizeof(binInfo));
        if (read < sizeof(binInfo)) {
            return (MDS_BOOT_RESULT_EIO);
        }

        uint16_t check =
            ALGO_CRC16(0, (uint8_t *)(&(binInfo.flag)), sizeof(binInfo) - sizeof(binInfo.check));
...
static MDS_BOOT_Result_t BOOT_UpgradeSwtich(MDS_BOOT_Device_t *dst, MDS_BOOT_Device_t *src,
                                            uint32_t srcOfs, uint32_t srcSize, uint16_t flag)
{
    ...
    switch (flag) {
        case MDS_BOOT_FLAG_COPY:
            ...
            break;
#if (defined(CONFIG_MDS_BOOT_UPGRADE_WITH_LZMA) && (CONFIG_MDS_BOOT_UPGRADE_WITH_LZMA != 0))
...
static MDS_BOOT_Result_t BOOT_UpgradeBinInfo(MDS_BOOT_Device_t *dst, MDS_BOOT_Device_t *src,
                                             size_t srcOfs,
                                             const MDS_BOOT_UpgradeInfo_t *upgradeInfo)
{
    uint16_t cnt = ALGO_GetU16BE(upgradeInfo->count);
...
MDS_BOOT_Result_t MDS_BOOT_UpgradeCheck(MDS_BOOT_SwapInfo_t *swapInfo, MDS_BOOT_Device_t *dst,
...
MDS_BOOT_SwapInfo_t *MDS_BOOT_GetSwapInfo(void)
{
    ...
    if ((swapInfo->magic != CONFIG_MDS_BOOT_UPGRADE_MAGIC) ||
        (swapInfo->check !=
         ALGO_CRC16(0, (uint8_t *)(&(swapInfo->magic)),
                    sizeof(*swapInfo) - sizeof(swapInfo->check)))) {
...
{
    size_t erase = MDS_BOOT_DeviceErase(dst);
    if (erase == 0) {
        return (MDS_BOOT_RESULT_EIO);
    }
    ...
    while (readOfs < srcSize) {
        ...
        size_t read = MDS_BOOT_DeviceRead(src, srcOfs + readOfs, g_bootCheckBuff, single);
        if (read < single) {
            return (MDS_BOOT_RESULT_EIO);
        }

        size_t write = MDS_BOOT_DeviceWrite(dst, srcOfs + readOfs, g_bootCheckBuff, single);
        if (write < single) {
            return (MDS_BOOT_RESULT_EIO);
        }
