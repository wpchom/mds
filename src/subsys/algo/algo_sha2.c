...
static void SHA256_SHA224_Process(uint32_t state[8], const uint8_t data[64])
{
    ...
    for (i = 0; i < 64; i++) {
        if (i < 16) {
            ...
            local.W[i] = SHA256_S1(local.W[i - 2]) + local.W[i - 7] + SHA256_S0(local.W[i - 15]) +
                local.W[i - 16];
        }

        local.temp1 = local.A[7] + SHA256_S3(local.A[4]) + F1(local.A[4], local.A[5], local.A[6]) +
            SHA256_K[i] + local.W[i];
...
static void SHA512_SHA384_Process(uint64_t state[8], const uint8_t data[64])
{
    ...
    for (i = 0; i < 80; i++) {
        if (i < 16) {
            ...
            local.W[i] = SHA512_S1(local.W[i - 2]) + local.W[i - 7] + SHA512_S0(local.W[i - 15]) +
                local.W[i - 16];
        }

        local.temp1 = local.A[7] + SHA512_S3(local.A[4]) + F1(local.A[4], local.A[5], local.A[6]) +
            SHA512_K[i] + local.W[i];
