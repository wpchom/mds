 ...
 **/
/* Include ----------------------------------------------------------------- */
#include "mds/device/spi.h"

/* SPI adaptr -------------------------------------------------------------- */
MDS_Err_t DEV_SPI_AdaptrInit(DEV_SPI_Adaptr_t *spi, const char *name,
                             const DEV_SPI_Driver_t *driver, MDS_DevHandle_t handle, MDS_Arg_t init)
{
...
{
    DEV_SPI_Periph_t *periph = (DEV_SPI_Periph_t *)MDS_DevPeriphCreate(
        sizeof(DEV_SPI_Periph_t), name, (MDS_DevAdaptr_t *)spi);
...
MDS_Err_t DEV_SPI_PeriphTransferMsg(DEV_SPI_Periph_t *periph, const DEV_SPI_Msg_t *msg)
{
    ...
    for (size_t retry = 0; (!MDS_ErrIsSame(err, MDS_EOK)) && (retry <= periph->object.retry);
         retry++) {
