...
static MDS_Err_t WORKQ_SkipListDrain(MDS_WorkQueue_t *workq, MDS_Lock_t *lock)
{
    ...
    while (!MDS_DListIsEmpty(&(workq->list[ARRAY_SIZE(workq->list) - 1]))) {
        ...
        if (workn->stop != NULL) {
            MDS_SpinLockRelease(&(workq->spinlock));
            MDS_CoreInterruptRestore(*lock);

            workn->stop(workn, workn->arg);

            *lock = MDS_CoreInterruptLock();
            err = MDS_SpinLockAcquire(&(workq->spinlock));
            if (err != MDS_EOK) {
                break;
            }
        }
...
static void WorkQueueThreadEntry(MDS_Arg_t *arg)
{
    ...
    while (err == MDS_EOK) {
        ...
        do {
            ...
            err = MDS_WorkQueueCheck(workq, &lock);
            if (err != MDS_EOK) {
                break;
            }
    ...
    MDS_LOG_E("[workq] thread err=%d exit", err);
}
...
MDS_Err_t MDS_WorkQueueDeInit(MDS_WorkQueue_t *workq)
{
    ...
    MDS_Lock_t lock = MDS_CoreInterruptLock();
    MDS_Err_t err = MDS_SpinLockAcquire(&(workq->spinlock));
    if (err == MDS_EOK) {
        MDS_Thread_t *thread = workq->thread;
        err = WORKQ_SkipListDrain(workq, &lock);
        if (err == MDS_EOK) {
