...
MDS_Err_t MDS_MemPoolDeInit(MDS_MemPool_t *memPool)
{
    ...
    MDS_Err_t err = MDS_SpinLockAcquire(&(memPool->spinlock));
    if (err == MDS_EOK) {
        MDS_KernelWaitQueueDrain(&(memPool->queueWait));
        err = MDS_ObjectDeInit(&(memPool->object));
    }
    if (err != MDS_EOK) {
        MDS_SpinLockRelease(&(memPool->spinlock));
    }
...
MDS_Err_t MDS_MemPoolDestroy(MDS_MemPool_t *memPool)
{
    ...
    if (err == MDS_EOK) {
        ...
        MDS_KernelWaitQueueDrain(&(memPool->queueWait));
        err = MDS_ObjectDestroy(&(memPool->object));
        if (err == MDS_EOK) {
