...
MDS_Err_t MDS_EventDeInit(MDS_Event_t *event)
{
    ...
    MDS_Err_t err = MDS_SpinLockAcquire(&(event->spinlock));
    if (err == MDS_EOK) {
        MDS_KernelWaitQueueDrain(&(event->queueWait));
        err = MDS_ObjectDeInit(&(event->object));
    }
    if (err != MDS_EOK) {
        MDS_SpinLockRelease(&(event->spinlock));
    }
...
MDS_Err_t MDS_EventDestroy(MDS_Event_t *event)
{
    ...
    if (err == MDS_EOK) {
        MDS_KernelWaitQueueDrain(&(event->queueWait));
        err = MDS_ObjectDestroy(&(event->object));
    }
    if (err != MDS_EOK) {
        MDS_SpinLockRelease(&(event->spinlock));
    }
                        ...
                        MDS_Timeout_t timeout)
{
    ...
    MDS_HOOK_CALL(KERNEL, event, (event, MDS_KERNEL_TRACE_EVENT_TRY_ACQUIRE, MDS_EOK, timeout));
    ...
    if (err == MDS_EOK) {
        if ((((opt & MDS_EVENT_OPT_AND) != 0U) &&
             ((event->value.mask & wait.mask) == wait.mask)) ||
...
MDS_Err_t MDS_EventSet(MDS_Event_t *event, MDS_Mask_t set)
{
    ...
    MDS_HOOK_CALL(KERNEL, event,
                  (event, MDS_KERNEL_TRACE_EVENT_HAS_SET, MDS_EOK, MDS_TIMEOUT_TICKS(mask)));
