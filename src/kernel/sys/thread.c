...
static void THREAD_Timeout(const MDS_Timer_t *timer, MDS_Arg_t arg)
{
    ...
    if (state == MDS_THREAD_STATE_SUSPENDED) {
        ...
        MDS_LOG_W("thread is not suspended");
    }
...
static MDS_Err_t MDS_ThreadClose(MDS_Thread_t *thread)
{
    ...
    MDS_Err_t err = THREAD_Terminate(thread);
...
MDS_Err_t MDS_ThreadDelay(MDS_Timeout_t timeout)
{
    ...
    MDS_ASSERT(thread != NULL);
    if ((thread == NULL) || (thread == MDS_KernelIdleThread())) {
        MDS_LOG_E("[thread] thread(idle/null) try delay:%" PRIuTICK " failed", timeout.ticks);
        MDS_ClockDelayRaw(timeout);
        return (MDS_EFAULT);
    }
    ...
    if (timeout.ticks == MDS_CLOCK_TICK_NO_WAIT) {
        ...
        MDS_SysTimerStart(&(thread->timer), timeout, MDS_TIMEOUT_NOWAIT);
    }
    ...
    if (MDS_ErrIsSame(thread->err, MDS_ETIME)) {
        thread->err = MDS_EOK;
    }
