...
MDS_Err_t MDS_SemaphoreAcquire(MDS_Semaphore_t *semaphore, MDS_Timeout_t timeout)
{
    ...
    MDS_LOG_D("[semaphore] thread(%p) acquire semephore(%p) err:%d, value:%" PRIuPTR, thread,
              semaphore, err.errno, semaphore->value);
...
MDS_Err_t MDS_SemaphoreRelease(MDS_Semaphore_t *semaphore)
{
    ...
    MDS_LOG_D("[semephore] release semephore(%p) value:%" PRIuPTR, semaphore, semaphore->value);
    ...
    MDS_HOOK_CALL(KERNEL, semaphore,
                  (semaphore, MDS_KERNEL_TRACE_SEMAPHORE_HAS_RELEASE, err, MDS_TIMEOUT_NOWAIT));
