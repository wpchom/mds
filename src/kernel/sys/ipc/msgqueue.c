...
static void MDS_MsgQueueListInit(MDS_MsgQueue_t *msgQueue, size_t msgNums)
{
    ...
    for (size_t idx = 0; idx < msgNums; idx++) {
        MDS_MsgQueueHeader_t *list = (MDS_MsgQueueHeader_t *)(&((
            (uint8_t *)(msgQueue
                            ->queBuff))[idx * (sizeof(MDS_MsgQueueHeader_t) + msgQueue->msgSize)]));
...
MDS_Err_t MDS_MsgQueueDeInit(MDS_MsgQueue_t *msgQueue)
{
    ...
    MDS_Lock_t lock = MDS_CriticalLock(&(msgQueue->spinlock));
...
MDS_Err_t MDS_MsgQueueRecvAcquire(MDS_MsgQueue_t *msgQueue, void *recv, MDS_Timeout_t timeout)
{
    ...
    if (msgQueue->lhead == NULL) {
        if (timeout.ticks == MDS_CLOCK_TICK_NO_WAIT) {
            ...
            MDS_LOG_D(
                "[msgqueue] msgqueue(%p) recv suspend thread(%p) entry:%p timer wait:%" PRIuTICK,
                msgQueue, thread, thread->entry, timeout.ticks);
        }
    }

    while ((MDS_ErrIsSame(err, MDS_EOK)) && (msgQueue->lhead == NULL)) {
                               ...
                               MDS_Timeout_t timeout)
{
    ...
    if (MDS_ErrIsSame(err, MDS_EOK)) {
        ...
        size_t rcnt = (rlen <= size) ? (rlen) : (size);
        memcpy(buff, recv, rcnt);
    }
                              ...
                              MDS_Timeout_t timeout)
{
    ...
    if (msgQueue->lfree == NULL) {
        if (timeout.ticks == MDS_CLOCK_TICK_NO_WAIT) {
            ...
            MDS_LOG_D(
                "[msgqueue] msgqueue(%p) send suspend thread(%p) entry:%p timer wait:%" PRIuTICK,
                msgQueue, thread, thread->entry, timeout.ticks);
        }
    }

    while ((MDS_ErrIsSame(err, MDS_EOK)) && (msgQueue->lfree == NULL)) {
    ...
    if (!MDS_ErrIsSame(err, MDS_EOK)) {
        return ((MDS_ErrIsSame(err, MDS_ETIME)) ? (MDS_ERANGE) : (err));
    }

    MDS_MsgListCopyBuff(msg + 1, msgQueue->msgSize, msgList);

    MDS_LOG_D("[msgqueue] send message to msgqueue(%p) len:%" PRIuPTR, msgQueue, len);

    lock = MDS_CriticalLock(&(msgQueue->spinlock));
...
MDS_Err_t MDS_MsgQueueUrgentMsg(MDS_MsgQueue_t *msgQueue, const MDS_MsgList_t *msgList)
{
    ...
    MDS_HOOK_CALL(KERNEL, msgqueue,
                  (msgQueue, MDS_KERNEL_TRACE_MSGQUEUE_TRY_SEND, MDS_EOK, MDS_TIMEOUT_NOWAIT));
    ...
    MDS_LOG_D("[msgqueue] send urgent message to msgqueue(%p) len:%" PRIuPTR, msgQueue, len);
