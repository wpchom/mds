...
MDS_Err_t MDS_RwLockDeInit(MDS_RwLock_t *rwlock)
{
    ...
    MDS_Err_t err = MDS_MutexAcquire(&(rwlock->mutex), MDS_TIMEOUT_NOWAIT);
    if (!MDS_ErrIsSame(err, MDS_EOK)) {
        return (err);
    }
    ...
    if ((rwlock->readers != 0) || (MDS_KernelWaitQueuePeek(&(rwlock->condWr.queueWait)) != NULL) ||
        ...
        err = MDS_SemaphoreAcquire(&(rwlock->condRd), MDS_TIMEOUT_NOWAIT);
        if (MDS_ErrIsSame(err, MDS_EOK)) {
            err = MDS_SemaphoreAcquire(&(rwlock->condWr), MDS_TIMEOUT_NOWAIT);
            if (MDS_ErrIsSame(err, MDS_EOK)) {
