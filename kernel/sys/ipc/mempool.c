...
static void MDS_MemPoolListInit(MDS_MemPool_t *memPool, size_t blkNums)
{
    ...
    for (size_t idx = 0; idx < blkNums; idx++) {
        union MDS_MemPoolHeader *list = (union MDS_MemPoolHeader *)(&(
            ((uint8_t *)(memPool->memBuff))[idx *
                                            (sizeof(union MDS_MemPoolHeader) + memPool->blkSize)]));
...
void *MDS_MemPoolAlloc(MDS_MemPool_t *memPool, MDS_Timeout_t timeout)
{
    ...
    if (memPool->lfree == NULL) {
        if (timeout.ticks == MDS_CLOCK_TICK_NO_WAIT) {
            ...
            MDS_LOG_D("[mempool] mempool(%p) alloc blocksize:%" PRIuPTR
                      " suspend thread(%p) entry:%p timer wait:%" PRIuTICK,
                      memPool, memPool->blkSize, thread, thread->entry, timeout.ticks);
        }
    }

    while ((MDS_ErrIsSame(err, MDS_EOK)) && (memPool->lfree == NULL)) {
    ...
    return (((MDS_ErrIsSame(err, MDS_EOK)) && (blk != NULL)) ? ((void *)(blk + 1)) : (NULL));
}
...
static void MDS_MemPoolFreeBlk(MDS_MemPool_t *memPool, union MDS_MemPoolHeader *blk)
{
    ...
    MDS_LOG_D("[mempool] free block to mempool(%p) which blksize:%" PRIuPTR, memPool,
              memPool->blkSize);

    MDS_HOOK_CALL(KERNEL, mempool,
                  (memPool, MDS_KERNEL_TRACE_MEMPOOL_HAS_FREE, MDS_EOK, MDS_TIMEOUT_NOWAIT, blk));
