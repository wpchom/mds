...
{
    return (CONTAINER_OF(MDS_ObjectFind(MDS_OBJECT_TYPE_DEVICE, name), MDS_Device_t, object));
}
                                     ...
                                     const MDS_Arg_t *init)
{
    ...
    if (module != NULL) {
        ...
        MDS_ObjectDestory(&(module->device.object));
    }
                                     ...
                                     const MDS_Arg_t *init)
{
    ...
    if (adaptr != NULL) {
        ...
        size_t handlesz = 0;
        driver->control(&(adaptr->device), MDS_DEVICE_CMD_HANDLESZ, (MDS_Arg_t *)(&handlesz));
        if (handlesz > 0) {
        ...
        MDS_ObjectDestory(&(adaptr->device.object));
    }
...
MDS_Err_t MDS_DevPeriphDestroy(MDS_DevPeriph_t *periph)
{
    ...
    MDS_Err_t err = MDS_DevPeriphClose(periph);
    if (err == MDS_EOK) {
        MDS_ObjectDestory(&(periph->device.object));
    }
...
MDS_Err_t MDS_DevPeriphOpen(MDS_DevPeriph_t *periph, MDS_Tick_t timeout)
{
    ...
    err = MDS_MutexAcquire(&(adaptr->mutex), timeout);
    if (err != MDS_EOK) {
        return (err);
    }
                                     ...
                                     const MDS_DevProbeTable_t drvList[], size_t drvSize)
{
    ...
    for (size_t idx = 0; idx < drvSize; idx++) {
        ...
        if (drvList[idx].driver->control(device, MDS_DEVICE_CMD_PROBE, (MDS_Arg_t *)device) != MDS_EOK) {
            continue;
        }
        if (driver != NULL) {
...
MDS_Err_t MDS_DevModuleDump(const MDS_Device_t *device, MDS_DevDumpData_t *dump)
{
    ...
    MDS_Err_t err = MDS_EIO;
    if ((module->driver != NULL) && (module->driver->control != NULL)) {
        err = module->driver->control(device, MDS_DEVICE_CMD_DUMP, (MDS_Arg_t *)dump);
    }
