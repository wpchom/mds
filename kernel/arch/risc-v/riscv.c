...
{
    register MDS_Lock_t lock = MDS_CoreInterruptLock();
...
static void CORE_StackBacktrace(uintptr_t stackPoint, uintptr_t stackLimit)
{
    for (size_t dp = 0; (dp < CONFIG_MDS_BACKTRACE_DEPTH) && (stackPoint < stackLimit);
        ...
        if (pc != 0) {
            MDS_LOG_P("[BACKTRACE] %" PRIuPTR ": %p", dp, (void *)pc);
            dp += 1;
        }
    }
}
...
static void CORE_ExceptionBacktrace(uintptr_t mcause, uintptr_t mscratch, uintptr_t sp)
{
    ...
    if (thread != NULL) {
        MDS_LOG_P("backtrace thread(%p) entry:%p sp:%p stackbase:%p stacksize:%" PRIuPTR, thread,
                  thread->entry, (void *)sp, thread->stackBase, thread->stackSize);
...
void MDS_CorePanicTrace(void)
{
    ...
    register uintptr_t mcause, mscratch, sp;
    __asm volatile("csrr        %0, mcause" : "=r"(mcause));
