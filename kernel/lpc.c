...
static void LPC_DeviceSuspend(MDS_LPC_Sleep_t sleep)
{
    ...
    MDS_DLIST_CONTAINER_FOREACH_NEXT (iter, node, &g_lpcDevList) {
...
static void LPC_DeviceResume(MDS_LPC_Run_t run)
{
    ...
    MDS_DLIST_CONTAINER_FOREACH_PREV (iter, node, &g_lpcDevList) {
...
/* PowerManager ------------------------------------------------------------ */
#if (defined(CONFIG_MDS_LPC_THREAD_IDLE) && (CONFIG_MDS_LPC_THREAD_IDLE > 0))
...
MDS_Err_t MDS_LPC_RunModeWait(MDS_LPC_Run_t run, MDS_Timeout_t timeout)
{
    ...
    MDS_LOG_W("[lpc] wait run:%d curr:%d timeout:%" PRIuTICK " err:%d", run, g_lpcMgr.runMode,
              timeout.ticks, err.errno);
