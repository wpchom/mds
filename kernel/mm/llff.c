...
static MDS_Err_t MDS_MemHeapLLFF_Setup(MDS_MemHeap_t *memheap, void *heapBase, size_t heapSize)
{
    ...
    MDS_LOG_D("[memory] memheap(%p) init begin:%p limit:%p size:%" PRIuPTR, memheap,
              (void *)alignBegin, (void *)alignLimit, alignLimit - alignBegin);
...
static void MemHeapLLFF_NodeFree(MDS_MemHeap_t *memheap, MemHeapLLFF_Node_t *node)
{
    ...
    MDS_LOG_D("[memory] memheap(%p) free node:%p size:%" PRIuPTR, memheap, node,
              (uintptr_t)(node->next) - (uintptr_t)(node) - sizeof(MemHeapLLFF_Node_t));
}

static void MDS_MemHeapLLFF_Free(MDS_MemHeap_t *memheap, void *ptr)
{
    MemHeapLLFF_Node_t *node =
        (MemHeapLLFF_Node_t *)((uint8_t *)(ptr) - sizeof(MemHeapLLFF_Node_t));
    if (node == NULL) {
        return;
    }
...
static MemHeapLLFF_Node_t *MemHeapLLFF_NodeAlloc(MDS_MemHeap_t *memheap, size_t alignSize)
{
    ...
    for (MemHeapLLFF_Node_t *node = lfree; node != limit; node = node->next) {
        ...
        MDS_LOG_D("[memory] memheap(%p) first:%p alloc node:%p size:%" PRIuPTR, memheap,
                  limit->next, node, alignSize);

        return (node);
    }

    MDS_LOG_D("[memory] memheap(%p) first:%p alloc size:%" PRIuPTR " no memory", memheap,
              limit->next, alignSize);
...
static MemHeapLLFF_Node_t *MemHeapLLFF_NodeRealloc(MDS_MemHeap_t *memheap, MemHeapLLFF_Node_t *node,
                                                   size_t alignSize)
{
    ...
    if (hopeSize > nodeSize) {
        if (MemHeapLLFF_NodeIsUsed(node->next) ||
            ...
            if (next != NULL) {
                memcpy((uint8_t *)next + sizeof(MemHeapLLFF_Node_t),
            ...
            MDS_LOG_D("[memory] memheap(%p) realloc node:%p extend size:%" PRIuPTR "->%" PRIuPTR,
                      memheap, node, nodeSize, hopeSize);
        }
    } else {
        MDS_HOOK_CALL(KERNEL, memheap,
                      (memheap, MDS_KERNEL_TRACE_MEMHEAP_REALLOC,
        ...
        MDS_LOG_D("[memory] memheap(%p) realloc node:%p reduce size:%" PRIuPTR "->%" PRIuPTR,
                  memheap, node, nodeSize, hopeSize);
    }
...
static void *MDS_MemHeapLLFF_Realloc(MDS_MemHeap_t *memheap, void *ptr, size_t size)
{
    ...
    if ((!MemHeapLLFF_IsPtrInside(memheap, ptr)) || (alignSize > totalSize)) {
        MDS_LOG_E("[memory] memheap(%p) realloc ptr(%p) size:%" PRIuPTR " error of total:%" PRIuPTR,
                  memheap, ptr, alignSize, totalSize);
        return (NULL);
    }
