...
MDS_FileOffset_t MDS_FileSeek(MDS_FileDesc_t *fd, MDS_FileOffset_t offset)
{
    MDS_ASSERT(fd != NULL);
    ...
    fd->pos = fd->node->fs->ops->seek(fd, offset);

    return (fd->pos);
}

MDS_Err_t MDS_FileTruncate(MDS_FileDesc_t *fd, MDS_FileOffset_t len)
{
    MDS_ASSERT(fd != NULL);
