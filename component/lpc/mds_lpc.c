...
static void LPC_SleepModeSwitch(MDS_LPC_Manager_t *mgr)
{
...
#endif
        MDS_LOG_E("[lpc] resume to run:%u fail but run:%u after sleep:%u", run, mgr->runMode, sleepMode);
    }
    ...
    MDS_LOG_W("[lpc] sleepMode:%u plan:%u real:%u resume runMode:%d", sleepMode, planSleep, realSleep, mgr->runMode);
}
...
/* PowerManager ------------------------------------------------------------ */
void MDS_KernelIdleLowPowerControl(void)
{
...
void MDS_LPC_SleepModeRequest(MDS_LPC_Sleep_t sleep)
{
    MDS_ASSERT(sleep < MDS_LPC_SLEEP_NUMS);
...
void MDS_LPC_RunModeRequest(MDS_LPC_Run_t run)
{
...
void MDS_LPC_RunModeRelease(MDS_LPC_Run_t run)
{
    MDS_ASSERT(run < MDS_LPC_RUN_NUMS);
...
MDS_Err_t MDS_LPC_RunModeWait(MDS_LPC_Run_t run, MDS_Tick_t timeout)
{
    ...
    MDS_LOG_W("[lpc] wait run:%u curr:%u timeout:%u err:%d", run, g_lpcMgr.runMode, timeout, err);
...
MDS_Err_t MDS_LPC_DeviceUnregister(MDS_LPC_Device_t *device)
{
    ...
    MDS_ListRemoveNode(&(device->node));
...
void MDS_LPC_RunTimeClear(void)
{
    ...
    g_lpcMgr.lastTick = MDS_SysTickGetCount();
