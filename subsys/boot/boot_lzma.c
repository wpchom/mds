...
#define CONFIG_MDS_BOOT_LZMA_ALLOC G_LZMA_ALLOC
#endif
                                                ...
                                                size_t srcSize, uint64_t unpackSize)
{
    ...
    for (size_t cnt = 0;; cnt++) {
        if (inPos == inSize) {
            inSize = ((srcSize - rIndex) > sizeof(g_lzmaReadBuff)) ? (sizeof(g_lzmaReadBuff))
                                                                   : (srcSize - rIndex);
        ...
        SRes res = LzmaDec_DecodeToBuf(dec, g_lzmaWriteBuff + outPos, &outProcessed,
                                       g_lzmaReadBuff + inPos, &inProcessed, finishMode, &status);
                                       ...
                                       uint32_t srcOfs, uint32_t srcSize)
{
    ...
    if (LzmaDec_Allocate(&dec, header, LZMA_PROPS_SIZE, &CONFIG_MDS_BOOT_LZMA_ALLOC) != SZ_OK) {
    ...
    MDS_BOOT_Result_t result =
        BOOT_LzmaDecodeUpgrade(&dec, dst, src, srcOfs + sizeof(header), srcSize, unpackSize);
    LzmaDec_Free(&dec, &CONFIG_MDS_BOOT_LZMA_ALLOC);
