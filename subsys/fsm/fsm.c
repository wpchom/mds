 ...
 **/
/* Include ----------------------------------------------------------------- */
#include "mds/fsm/fsm.h"

/* Funtcion ---------------------------------------------------------------- */
MDS_Tick_t MDS_FSM_TaskManagerEntry(MDS_FSM_TaskManager_t *taskManager)
{
    MDS_Tick_t tickdiff = (MDS_Tick_t)(-1);

    if (taskManager == NULL) {
        return (tickdiff);
    }

    while (!MDS_DListIsEmpty(&(taskManager->ready))) {
        MDS_FSM_TaskNode_t *taskNode =
            CONTAINER_OF(taskManager->ready.next, MDS_FSM_TaskNode_t, node);
    ...
    if (!MDS_DListIsEmpty(&(taskManager->block))) {
        MDS_FSM_TaskNode_t *taskNode =
            CONTAINER_OF(taskManager->block.next, MDS_FSM_TaskNode_t, node);
...
void MDS_FSM_TaskNodeSetupReady(MDS_FSM_TaskNode_t *taskNode, MDS_FSM_TaskManager_t *taskManager)
{
    ...
    if (MDS_DListIsEmpty(&(taskNode->taskManager->ready))) {
        MDS_DListInitNode(&(taskNode->taskManager->ready));
    }

    MDS_DListRemoveNode(&(taskNode->node));

    MDS_FSM_TaskNode_t *iterTask = NULL;
    MDS_DLIST_CONTAINER_FOREACH_NEXT (iterTask, node, &(taskNode->taskManager->ready)) {
        if (iterTask->priority > taskNode->priority) {
            MDS_DListInsertNodePrev(&(iterTask->node), &(taskNode->node));
            break;
        }
    }
}
...
void MDS_FSM_TaskNodeWaitBlock(MDS_FSM_TaskNode_t *taskNode, MDS_Timeout_t timeout)
{
    ...
    if (MDS_DListIsEmpty(&(taskNode->taskManager->block))) {
        MDS_DListInitNode(&(taskNode->taskManager->block));
    }

    MDS_DListRemoveNode(&(taskNode->node));

    MDS_Tick_t tickcurr = MDS_ClockGetTickCount();
    taskNode->ticknext = tickcurr + timeout.ticks;

    MDS_FSM_TaskNode_t *iterTask = NULL;
    MDS_DLIST_CONTAINER_FOREACH_NEXT (iterTask, node, &(taskNode->taskManager->block)) {
                                   ...
                                   uint8_t pollStep, uint8_t overStep)
{
    ...
    uint8_t event = taskNode->waitEvent;
    if (event == 0) {
        if ((taskNode->pollCount > 0) && (timeout.ticks > 0)) {
            taskNode->pollCount -= 1;
            MDS_FSM_TaskNodeWaitBlock(taskNode, timeout);
